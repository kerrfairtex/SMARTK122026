#!/bin/bash
# Watch for the landing page OR a /login.php 200 (either proves the swap took effect).
end=$(( $(date +%s) + 1500 ))
while [ $(date +%s) -lt $end ]; do
  ...
  if [ "$loginhp" = "200" ]; then
    printf '[%s] swap partially live: /login.php=200 (login renamed). root still size=%s\n' "$(date +%H:%M:%S)" "$size"
  fi
